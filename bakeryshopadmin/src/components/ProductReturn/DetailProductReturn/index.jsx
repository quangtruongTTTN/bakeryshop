...
const DetailProductReturn = () => {
    const classes = useStyles();
    const location = useLocation();
    const [productReturn] = useState(location?.state?.productReturn);
    // const [status, setStatus] = useState(productReturn.status);
    const auth = useSelector((state) => state.auth);
    const dispatch = useDispatch();
    const history = useHistory();

    // const [shipper, setShipper] = useState(productReturn.shipperId);
    ...
    const handleUpdate = (id, status, shipper) => {
        if(shipper === null){
            confirmAlert({
                title: "CHỌN SHIPPER CHO CHI TIẾT PHIẾU TRẢ",
        ...
        <React.Fragment>
            ...
            <main className={classes.layout}>
                <Paper className={classes.paper}>
                    ...
                    {/* <PDFDownloadLink
                        document={<Report productReturn={productReturn} />}
                        fileName="report"
                    >
                    ...
                    <Typography variant="h6" className={classes.title}>
                        <Grid container spacing={3}>
                            <Grid item md={12} xl={12} sm={12} className={classes.wrapBarCode}>Mã phiếu nhập: <BarCode value={productReturn.id} /></Grid>
                            <Grid item md={6} xl={6} sm={12}>Ngày lập: {moment(productReturn.createdAt).format("YYYY-MM-DD")}</Grid>
                            {/* <Grid item md={6} xl={6} sm={12}>Lưu ý thêm: {productReturn.noteOrder ? productReturn.noteOrder : "Không có lưu ý"}</Grid> */}
                            {/* <Grid item md={6} xl={6} sm={12}>Phương thức thanh toán:
                                {productReturn.payment === 1 && (
                                    <span
                            ...
                            <Grid item md={6} xl={6} sm={12}>Nhân viên tạo: {productReturn.employeeName}</Grid>
                            <Grid item md={6} xl={6} sm={12}>Mã Nhân viên tạo: {productReturn.employeeId}</Grid>
                            {/* <Grid item md={12} xl={12} sm={12}>
                                Địa chỉ: {productReturn.employeeName}
                            </Grid>
                            <Grid item md={6} xl={6} sm={12}>Nhân viên duyệt: {productReturn?.employeeName}</Grid> */}
                            ...
                            <Grid item xs={12} md={12}>
                                <TableContainer component={Paper}>
                                    <Table className={classes.table} aria-label="spanning table">
                                        ...
                                        <TableBody>
                                            {productReturn?.productReturnDetails?.length > 0 ? productReturn?.productReturnDetails?.map((item) => (
                                            ...
                                            {/* <TableRow>
                                                ...
                                                <TableCell align="right" className={classes.cellWithoutBorder}>
                                                    <b style={{ paddingRight: 45 }} translate="no">{((productReturn.totalPrice - productReturn.shipping + productReturn.memberVip) / 1.05).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</b>
                                                </TableCell>
                                            </TableRow> */}
                                            {/* <TableRow>
                                                <TableCell colSpan={3} className={classes.cellWithoutBorder} />
                                                ...
                                                <TableCell align="right" className={classes.cellWithoutBorder}>
                                                    <b style={{ paddingRight: 45 }} translate="no">{((productReturn.totalPrice - productReturn.shipping + productReturn.memberVip) / 1.05 * 0.05).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</b>
                                                </TableCell>
                                            </TableRow> */}
                                            {/* <TableRow>
                                                <TableCell colSpan={3} className={classes.cellWithoutBorder} />
                                                ...
                                                <TableCell align="right" className={classes.cellWithoutBorder}>
                                                    <b style={{ paddingRight: 45 }} translate="no">{(productReturn.shipping).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</b>
                                                </TableCell>
                                            </TableRow> */}
                                            {/* <TableRow>
                                                <TableCell colSpan={3} />
                                                ...
                                                <TableCell align="right">
                                                    <b style={{ paddingRight: 45 }} translate="no">{(productReturn.memberVip).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</b>
                                                </TableCell>
                                            </TableRow> */}
                                            {/* <TableRow>
                                                <TableCell colSpan={3} />
                                                ...
                                                <TableCell align="right">
                                                    <b style={{ fontSize: 20, paddingRight: 45 }} translate="no">{(productReturn.totalPrice).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</b>
                                                </TableCell>
                                            </TableRow> */}
                                        </TableBody>
                                    </Table>
                                ...
                                {/* {(status === 1 || status === 2 || status === 5) && (<Button
                                    ...
                                    onClick={() => handleUpdate(productReturn.id, status, shipper)}
                                >
                                    CẬP NHẬT TRẠNG THÁI
                                </Button>)}
                                {status === 1 && (<Button
                                ...
                                <Button
                                    ...
                                    color="primary"
                                    className={classes.button}
                                    onClick={() => history.push("/productReturn")}
                                >
                                    Quay lại
                                </Button>
                            </Grid>
...
export default DetailProductReturn