        ...
        {order?.orderDetails.map((item, index) => (
          <View key={index}>
            <View style={styles.row}>
              ...
              <Text style={styles.price}>{item.priceCurrent.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VND</Text>
              <Text style={styles.price}>{item.quantity}</Text>
              <Text style={styles.price}>{(item.quantity * item.priceCurrent).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VND</Text>
            </View>
          </View>
        ))}
