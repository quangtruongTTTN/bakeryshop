import { TextField, Button, Grid, makeStyles, Typography, CssBaseline, Chip, Avatar, MenuItem, Select, FormControl, InputLabel } from "@material-ui/core"
...
import { ShipperAndNumberOrderGetAll } from "../../../store/actions/ShipperAction";
...
const HistoryDetail = () => {
    ...
    const [shipperName, setShipperName] = useState(order?.shipperId?.fullName);
    const [selected, setSelected] = useState(initialState);
    const [input, setInput] = useState('');
    const [isChange, setIsChange] = useState(false);
    const handleSelected = (event, value) => {
        Notification.success(JSON.stringify(value?.user));
        setIsChange(true);
        
        setShipperName(value?.user?.fullName);
        setShipper(value?.user);
        
        // return
        // setInput(value?.user?.fullName)
        
        // Notification.success(value?.user?.fullName);
        // return
        
    };
    const getFriendSelecter = (value) => {
        return shippers.filter((item) => item.value === value);
      };
    const handleChange = (id) => {
        dispatch(OrderStatusAndShipperUpdate({ id, status: order.status, shipperId: shipper.id, employeeId: 1 }));
        Notification.success(shipper.id);
    ...
    const handlePrint = async () => {
        if(order.invoice === null){
            Notification.success(JSON.stringify(order));
        }else{

        }
        Notification.success(JSON.stringify(order.invoice));
        return
    ...
    const handleDirect = async () => {
        ...
        try {
            const response = await InvoiceService.getInvoiceById(order.id);
    ...
    const handleComfirmedStatus = (id, shipperId) => {
        dispatch(OrderStatusAndShipperUpdate({ id, status: 5, shipperId: shipperId, employeeId: auth.user.id }));
                ...
                <Paper className={classes.paper}>
                    ...
                    <Typography variant="h6" className={classes.title}>
                        <Grid container spacing={3}>
                            ...
                            <Grid item md={12} xl={12} sm={12}>Nhân viên duyệt: {order?.employeeId?.fullName}</Grid>
                                ...
                                <FormControl>
                                    ...
                                    >
                                        {shippers.map((value, index) => {
                                            return <MenuItem value={value.user.fullName}>{value.user.fullName + ' , Đang giao: ' + value.numberOfOrder} </MenuItem>;
                                        })}
                                    </Select>
                                </FormControl>
                            ...
                            <Grid item md={12} xl={12} sm={12}>
                                ...
                                <Autocomplete
                                    freeSolo
                                    id="free-solo-demo"
                                    // options={shippers.map((value) => (value.fullName ))} 
                                    // inputValue={}
                                    options={shippers.map((value, index) => ({
                                        
                                        "user" : value?.user,
                                        "fullName" : value?.user?.fullName,
                                        "numberOfOrder" : value?.numberOfOrder
                                        // return <MenuItem value={value.user.fullName}>{value.user.fullName + ' , Đang giao: ' + value.numberOfOrder} </MenuItem>;
                                        // return value.user.fullName + ' , Đang giao: ' + value.numberOfOrder;
                                    }))}
                                    getOptionLabel={(option) => option.fullName}
                                    //if !(auth.user.roles.includes("ROLE_SHIPPER")
                                    // getOptionLabel={option => (option.fullName + ' , Đang giao: ' + option.numberOfOrder)}
                                    disabled={ (status === 3 || status === 2 || status === 4 || auth.user.roles.includes("ROLE_SHIPPER"))   ? true : false}
                                    onChange={(event, value) => handleSelected(event, value)}
                                    // onChange={handleSelected}
                                    defaultValue={{"fullName" :shipperName}}
                                    // value={{"fullName" :shipperName}}
                                    ...
                                    renderInput={params => (
                                        <TextField
                                            ...
                                            variant="outlined"
                                            fullWidth
                                            // onChange={({ target }) => setInput(target.value?.user?.fullName)}
                                            // onChange={({ target }) => setShipperName(target.value?.user?.fullName)}
                                        />
                                    )}
                                />
                                </Grid>
                                        ...
                                        <TableBody>
                                            {order?.orderDetails?.length > 0 ? order?.orderDetails?.map((item) => (
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell align="center" translate="no">{(item.priceCurrent).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</TableCell>
                                                    <TableCell align="center" translate="no">{(item.priceCurrent * item.quantity).toLocaleString('it-IT', { style: 'currency', currency: 'VND' })}</TableCell>
