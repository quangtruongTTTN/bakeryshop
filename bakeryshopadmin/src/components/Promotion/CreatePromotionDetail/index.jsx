...
import { PromotionDetailAddAction } from "../../../store/actions/PromotionDetailAction";
...
const CreatePromotionDetail = () => {
  ...
  const onSubmit = async (data) => {
    if(data.createdAt>data.endDate){
      Notification.success("Ngày bắt đầu phải sau ngày kết thúc!");
    }else{
      // setTimeout(() => {
      //   dispatch(PromotionDetailAddAction(data)).then((res) => {
      //     if(res==='OK'){
      //       // history.push("/promotiondetail");
      //       Notification.error("Thời gian sale bị trùng, Vui lòng chọn lại !");
      //     }else{
      //       history.push("/promotiondetail");
      //       Notification.success("Đã Giảm giá sản phẩm thành công!");
      //     }
      //   });  
      // }, 2000);Sản phẩm này có trong đợt giảm giá
      await dispatch(PromotionDetailAddAction(data)).then((res) => {
        // if(res.message==='OK'){
          if(res.message==="Sản phẩm này có trong đợt giảm giá"){
          // history.push("/promotiondetail");
          Notification.error("Sản phẩm này có trong đợt giảm giá, Vui lòng chọn sản phẩm khác !");
        }else{
  ...
  const discounts = [
    { id: 1, label: "10%", discount: 10 },
    { id: 2, label: "20%", discount: 20 },
    { id: 3, label: "30%", discount: 30 },
    { id: 4, label: "40%", discount: 40 },
    { id: 5, label: "50%", discount: 50 },
    { id: 6, label: "60%", discount: 60 },
    { id: 7, label: "70%", discount: 70 },
    { id: 8, label: "80%", discount: 80 },
    { id: 9, label: "90%", discount: 90 },
  ];
...
export default CreatePromotionDetail;
